...
const createGetAuth = ({
  ...
  return (req, opts) => {
    ...
    const signedIn = (0, import_backend.signedInAuthObject)(jwt.payload, { ...options, token: jwt.raw.text });
    if (signedIn.user) {
      (0, import_deprecated.deprecatedObjectProperty)(signedIn, "user", "Use `clerkClient.users.getUser` instead.");
    }
    if (signedIn.organization) {
      (0, import_deprecated.deprecatedObjectProperty)(signedIn, "organization", "Use `clerkClient.organizations.getOrganization` instead.");
    }
    if (signedIn.session) {
